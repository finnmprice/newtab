...
function randomBgColor() {
	...
	const validColors = randomColors.filter(
		(color) => color.trim() !== "" && validHexColorRegex.test(color.trim()),
	);
	let colorToUse;
	...
	do {
		...
		if (
			`rgb(${r},${g},${b})` !== currentBgColor ||
			attempts >= validColors.length
		) {
			updateColorDisplay(r, g, b);
...
function updateGradient() {
	...
	gradientInterval = setInterval(() => {
		step++;
		let midColor1 = interpolateColor(
			currentColor1,
			targetColor1,
			step / steps,
		);
		let midColor2 = interpolateColor(
			currentColor2,
			targetColor2,
			step / steps,
		);
...
function interpolateColor(colorStart, colorEnd, factor) {
	...
	let r = Math.ceil(
		parseInt(result.substring(0, 2), 16) * (1 - factor) +
			parseInt(resultEnd.substring(0, 2), 16) * factor,
	);
	let g = Math.ceil(
		parseInt(result.substring(2, 4), 16) * (1 - factor) +
			parseInt(resultEnd.substring(2, 4), 16) * factor,
	);
	let b = Math.ceil(
		parseInt(result.substring(4, 6), 16) * (1 - factor) +
			parseInt(resultEnd.substring(4, 6), 16) * factor,
	);
...
$("#rslider, #gslider, #bslider").on("input", function () {
	...
	UpdateValue(
		rgbToHex($("#rslider").val(), $("#gslider").val(), $("#bslider").val()),
	);
});
...
function updateTextWidth(id) {
	document.getElementById(id).style.width =
		document.getElementById(id).value.length + 1 + "ch";
}
			...
			for (const alternative of alternatives) {
				if (input.startsWith(alternative)) {
					...
					if (urlMappings.dynamic[mainKey]) {
						const dynamicComponent = restOfInput
							? " " + restOfInput
							: "";
